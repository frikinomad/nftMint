...
const MintNft: React.FC = () => {
    ...
    // Update form fields immediately on change
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      // Also update the nftDetail state
      setNftDetail(prev => ({
          ...prev,
          [id]: value
      }));
    ...
    const mintNft = async (metadataUri: string) => {
      ...
      try {
        ...
        return {status: mint.publicKey.toString(), error: false};
      } catch (err) {
        if (err instanceof SendTransactionError) {
          console.error('Transaction Error:', err);
          return {status: 'Transaction error', error: true}
        } else {
    ...
    const handleMintNft = useCallback(async () => {
      ...
      try {
        ...
        setMintAddress(nftMintAddress.status);
        setError(null);
      }catch (err: unknown) {
